...
class TicketCreationLineSkip: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        ref.child("stripe_bar_pricing").child(barID).observeSingleEvent(of: .value, with: { (snapshot) in
            ...
            if(snapshot.hasChild("line_skip"))
            {
                itemName = "LineSkip"
                print(data.key)
                price = (data.childSnapshot(forPath: "line_skip").value) as! String
                ...
                let ticket = TicketStruct(itemName: itemName,price: price,amountID: amount)!
