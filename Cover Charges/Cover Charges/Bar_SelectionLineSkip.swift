...
class Bar_SelectionLineSkip: UIViewController, UITableViewDelegate, UITableViewDataSource{
    var bars = [BarStruct]()
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        performSegue(withIdentifier: "toTicketCreationLineSkip", sender: self)
    }
    override func viewDidLoad() {
        ...
        ref.child("stripe_bar_users").observeSingleEvent(of: .value, with: { (snapshot) in
            for child in snapshot.children {
                ...
                let barID = data.key
                let isCover = data.childSnapshot(forPath: "line_skip").value as! Bool
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if(segue.identifier == "toTicketCreationLineSkip")
        {
            let dest = segue.destination as! TicketCreationLineSkip
