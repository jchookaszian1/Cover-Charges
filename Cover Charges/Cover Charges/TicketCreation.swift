...
class TicketCreation: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        var initialTotal = 0
        ref.child("stripe_bar_pricing").child(barID).child("menu_items").observeSingleEvent(of: .value, with: { (snapshot) in
            for child in snapshot.children {
                
                
                let data = child as! DataSnapshot
                let itemName = (data.childSnapshot(forPath: "name").value)! as! String
                let price = (data.childSnapshot(forPath: "price").value)! as! String
                ...
                guard let ticket = TicketStruct(itemName: itemName,price: price,amountID: amount) else {
